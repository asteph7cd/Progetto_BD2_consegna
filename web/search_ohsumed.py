...
def expq_cor(ix,query):
    ...
    for x in check:                                                         # Per ogni parola in check
        corr = []
        for y in corrector.suggest(x, prefix=3, maxdist=1,limit=1):         # Mantengo un prefisso di 3 lettere e ottengo la lista dei suggerimenti 
...
def suggerimenti(ix,query):                                                
    qt_correction = dict([(x[1],[]) for x  in query.all_terms(phrases=True) if len(x[1]) > 2])
